 ...
 * 依赖迁移 V4 创建 document_chunk 表与 vector 列；单测用内存实现替代。
 */
@Component
...
public class PgVectorStoreAdapter implements VectorStorePort {
    ...
    private static final String INSERT_SQL =
            "INSERT INTO document_chunk (document_id, chunk_index, content, embedding) VALUES (?, ?, ?, ?::vector)";
    private static final String SEARCH_SQL =
            "SELECT document_id, chunk_index, content, 1 - (embedding <=> ?::vector) AS score "
    ...
    public void put(DocumentChunk chunk, float[] vector) {
        jdbcTemplate.update(INSERT_SQL, chunk.documentId(), chunk.chunkIndex(), chunk.content(), formatVector(vector));
    }
