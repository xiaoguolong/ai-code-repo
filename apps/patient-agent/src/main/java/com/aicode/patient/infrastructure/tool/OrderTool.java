...
public class OrderTool implements Tool {
    ...
    public OrderTool(PatientDataPort patientData, ObjectMapper objectMapper) {
    ...
    public ToolDefinition definition() {
        return new ToolDefinition("OrderTool", "按患者编号查询医嘱列表", PatientToolSupport.patientIdSchema());
    }
    ...
    public ToolResult execute(Map<String, Object> arguments) {
        ...
        try {
            return new ToolResult(objectMapper.writeValueAsString(patientData.listOrders(patientId)));
        } catch (Exception ex) {
            throw new ToolExecutionException("failed to serialize orders", ex);
        }
    }
}
